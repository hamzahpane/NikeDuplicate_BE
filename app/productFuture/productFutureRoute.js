...
const controller = require("./productFutureControler");

// Konfigurasi Multer untuk upload file
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    // Direktori tujuan penyimpanan file
    cb(null, "public/images/productsFuture");
  },
...
// Endpoint untuk membuat product future
router.post(
  "/createFuture",
  upload.single("image_url"), // Middleware Multer untuk upload file
  (req, res, next) => {
    controller.createProductFuture(req, res, next).catch(next); // Menangani error dari async controller
  }
);

router.get("/getFuture", controller.getProductFuture);

router.get("/getFuture/:id", controller.getProductFutureById);
